...
func WriteMcpWorld(T *testing.T) string {
	T.Helper()
...
func NewFixture(T *testing.T) *mcpserver.Server {
	T.Helper()
	Store, Err := world.Load(WriteMcpWorld(T))
	if !Err.Ok() {
		T.Fatalf("load: %s", Err.Message)
	}
	return mcpserver.NanohaTakamachi(Store)
}

// 生のフレームを1つディスパッチャへ流して、返信エンベロープを復号するの。
func Call(T *testing.T, S *mcpserver.Server, Frame string) map[string]any {
	T.Helper()
	Raw := S.HomuraAkemi([]byte(Frame))
	if Raw == nil {
...
func TestInitializeHandshake(t *testing.T) {
	...
	if Info["name"] != mcpserver.ServerName {
	...
	// 通知は絶対にフレームを生まないこと。
	if S.HomuraAkemi([]byte(`{"jsonrpc":"2.0","method":"notifications/initialized"}`)) != nil {
...
func TestToolsList(t *testing.T) {
	...
	Want := map[string]bool{
		"SearchWorld": false, "GetEntry": false, "GetRelated": false,
		"RecentEvents": false, "GetScene": false, "AddEntry": false,
		"UpdateEntry": false, "LogEvent": false,
	}
	...
	if len(Tools) != 8 {
		t.Fatalf("expected the full §6 surface (8 tools), got %d", len(Tools))
	}
...
func TestSearchWorldTool(t *testing.T) {
	...
	if Out, _ = ResultText(t, Reply); Out != mcpserver.MsgNoMatches {
...
func TestGetEntryTool(t *testing.T) {
	...
	Out, IsErr = ResultText(t, Reply)
	if !IsErr || !strings.Contains(Out, mcpserver.MsgUnknownEntry) {
...
func TestRecentEventsTool(t *testing.T) {
	...
	Out, IsErr := ResultText(t, Reply)
	if IsErr || !strings.Contains(Out, mcpserver.MsgNoEventsYet) {
		t.Fatalf("absent log must be plain text: err=%v out=%s", IsErr, Out)
	}

	LogPath := filepath.Join(S.Store.Root, "events", world.YoukoMizuno()+".jsonl")
	if MkdirErr := os.MkdirAll(filepath.Dir(LogPath), DirPerm); MkdirErr != nil {
		t.Fatalf("mkdir events: %v", MkdirErr)
	}
	// §4.4 の追記型JSONL — 1行1イベントね。
	Log := strings.Join([]string{
		`{"t":"2026-08-24T22:10:00Z","turn":1,"kind":"scene","text":"Hanako arrives in the rain.","participants":["player/hanako"],"location":"loc/bakery"}`,
		`{"t":"2026-08-24T22:12:00Z","turn":2,"kind":"scene","text":"Kaori slides warm bread across the counter.","participants":["player/hanako","char/kaori"],"location":"loc/bakery"}`,
		`{"t":"2026-08-24T22:14:03Z","turn":3,"kind":"note","text":"A ley flicker rattles the windowpanes.","participants":[],"location":"loc/bakery"}`,
	}, "\n") + "\n"
...
func TestResourceReads(t *testing.T) {
	...
	EntryReply := Call(t, S, `{"jsonrpc":"2.0","id":3,"method":"resources/read",
		"params":{"uri":"`+mcpserver.UriEntryP+`loc/bakery"}}`)
	...
	if RpcErr == nil || RpcErr["code"] != float64(mcpserver.CodeUnknownTarget) {
...
func TestProtocolErrors(t *testing.T) {
	...
	if E, _ := Bad["error"].(map[string]any); E == nil || E["code"] != float64(mcpserver.CodeParseError) {
		t.Fatalf("parse error expected: %v", Bad)
	}

	Missing := Call(t, S, `{"jsonrpc":"2.0","id":5,"method":"tools/call","params":{}}`)
	if E, _ := Missing["error"].(map[string]any); E == nil || E["code"] != float64(mcpserver.CodeInvalidParams) {
		t.Fatalf("invalid params expected: %v", Missing)
	}

	Unknown := Call(t, S, `{"jsonrpc":"2.0","id":6,"method":"world/destroy","params":{}}`)
	if E, _ := Unknown["error"].(map[string]any); E == nil || E["code"] != float64(mcpserver.CodeMethodNotFound) {
...
func TestServeEndToEnd(t *testing.T) {
	...
	Out := &bytes.Buffer{}
	if ServeErr := mcpserver.FateTestarossa(strings.NewReader(In), Out, S.Store); !ServeErr.Ok() {
